...
import { Button, Empty, Loader, Modal, SearchBar } from "../../ui";
...
export const WorkflowTables = () => {
  ...
  const { data: dataWorkflows, loading: loadingWorkflows } = useFetch({
    endpoint: "workflows",
  });
  ...
  useEffect(() => {
    if (!loadingWorkflows) {
      if (hasSearch) {
        setDataSorted(
          [...dataSorted].sort((a: WorkflowType, b: WorkflowType) => {
        ...
        setDataSorted(
          [...dataWorkflows].sort((a: WorkflowType, b: WorkflowType) => {
            if (sortedAZ) return a.id - b.id;
            return b.id - a.id;
          })
        );
  ...
  const handleClick = (workspaceId: number) => {
    setSelectedJobs(
      jobs.filter(
        (job: JobType) =>
          job.workflow_id === workspaceId && job.status !== "success"
      )
    );
    setShowModal(true);
  };
  ...
  const handleSearch = (search: string) => {
    ...
    if (search !== "") {
      setLoadingSearch(true);
      setDataSorted(
        dataWorkflows.filter((element: WorkflowType) =>
  ...
  return (
    <div>
      <section className="filter">
        ...
        <Button
          onClick={() => setSortedAZ(!sortedAZ)}
          iconAfter={
            <FontAwesomeIcon
              className="icon"
              icon={sortedAZ ? faCaretDown : faCaretUp}
            />
          }
        >
          {sortedAZ ? "Sort A-Z" : "Sort Z-A"}
        </Button>
      </section>
      <table>
        <thead>
      ...
      {showModal && (
        <Modal jobs={selectedJobs} onClose={() => setShowModal(false)} />
      )}
    </div>
  );
};
