/*
    convertSecToMin:
        - function to convert raw second in human readable
        - two function for mobile and desktop
 */
...
export function secToMin(value: number): string {
  ...
  if (day > 0)
    return `${day} day, ${hour} hours, ${minutes} minutes,${seconds} seconds`;
...
export function secToMinMobile(value: number): string {
  ...
  if (day > 0)
    return `${day} day ${addZero(hour)}:${addZero(minutes)}:${addZero(
      seconds
    )}`;
  if (hour > 0)
